...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='didi', help='the dataset to be attacked')
...
def main():
    ...
    for i_batch, batch_data in enumerate(dataloader):
        ...
        for step in range(max_step):
            ...
            all_bbox_score_loss = (iou_mask  * cls_mask * iou.detach() * bbox_pred[:,4]).sum()
            ...
            if step % 10 == 0:
                print(step, 
                'det_loss:',
                round(float(all_bbox_score_loss),3), 
                'wh_loss:',
            ...
            total_loss = all_bbox_score_loss + extra_loss
            total_loss.backward()
        ...
        # save mask
        best_mask = attacker.best_mask
        save_path = os.path.join(save_dir, 'mask', adv_image_name)
        os.makedirs(os.path.dirname(save_path), exist_ok=True)
        save_image(best_mask, save_path)
